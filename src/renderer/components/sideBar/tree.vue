  ...
  <div class="tree-view">
    ...
    <div class="opened-files">
      <div class="title">
        ...
        <span class="default-cursor text-overflow" @click.stop="toggleOpenedFiles()">{{ $t('fileTree.openedFiles') }}</span>
        <a href="javascript:;" @click.stop="saveAll(false)" :title="$t('fileTree.saveAll')">
          <svg class="icon" aria-hidden="true">
            <use xlink:href="#icon-save-all"></use>
          </svg>
        </a>
        <a href="javascript:;" @click.stop="saveAll(true)" :title="$t('fileTree.closeAll')">
          <svg class="icon" aria-hidden="true">
    ...
    >
      ...
      <div class="tree-wrapper" v-show="showDirectories">
        ...
        <div class="empty-project" v-if="projectTree.files.length === 0 && projectTree.folders.length === 0">
          <span>{{ $t('fileTree.emptyProject') }}</span>
          <a href="javascript:;" @click.stop="createFile">{{ $t('fileTree.createFile') }}</a>
        </div>
      </div>
    </div>
